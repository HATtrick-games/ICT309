...
namespace AnimationSample
{
  ...
  // With another key the animations can be faded-out or faded-in.
  // A fade-in or a cross-fade is created by starting a new animation and specifying a Replace
  // transition with a fade-in time. 
  // A fade-out is created using AnimationController.Stop().
  // See Update() method.
  public class CrossFadeSample : Sample
  {
    private AnimatableProperty<Vector2> _animatedPosition = new AnimatableProperty<Vector2>();


    // Current state: 0 = no animation, 1 = horizontal animation, 2 = vertical animation
    private int _state;
    private AnimationClip<Vector2> _horizontalAnimation;
    private AnimationClip<Vector2> _verticalAnimation;
      ...
      : base(game)
    {
      DisplayMessage = "CrossFadeSample\n"
                     ...
                     + "Press <Back> to stop/start all animations (using fade-out/fade-in).";
    }
