...
namespace AnimationSample
{
  ...
  // you need to create a StringTraits class (see StringTraits.cs) that tells the animation 
  // service how strings are created/recycled/added/interpolated/blended/etc.
  // This StringTraits class can be used in a custom animation (see StringKeyFrameAnimation.cs).
  public class StringAnimationSample : Sample
  {
    private AnimatableProperty<string> _animatableString = new AnimatableProperty<string>();


    public StringAnimationSample(Game game)
      : base(game)
    {
      DisplayMessage = "StringAnimationSample\n"
                     + "This sample animates a string value.\n"
                     + "DigitalRune Animation does not have built-in support for string animations.\n"
                     + "This sample shows how to add a key-frame animation for strings.";
    }
