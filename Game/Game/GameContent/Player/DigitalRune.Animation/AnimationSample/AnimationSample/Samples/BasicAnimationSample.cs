...
{
  // This sample shows how to use a from/to/by animation to animate the x position of
  // a sprite. The sprite color is animated with a key-frame animation. Both animations
  // oscillate until the animation is stopped by the user.
  public class BasicAnimationSample : Sample
  {
    private AnimatableProperty<float> _animatableX = new AnimatableProperty<float> { Value = 300 };
    private AnimatableProperty<Color> _animatableColor = new AnimatableProperty<Color> { Value = Color.Black };

    private AnimationClip<float> _oscillatingXAnimation;
    private AnimationClip<Color> _loopedColorAnimation;


    public BasicAnimationSample(Game game)
      : base(game)
    {
      DisplayMessage = "BasicAnimationSample\n"
                     + "The x position of the sprite is animated using an oscillating from/to/by animation.\n"
                     + "The color of the sprite is animated using a looping key-frame animation.\n"
    ...
    {
      float minX = GraphicsDevice.Viewport.TitleSafeArea.Left + 100;
      float maxX = GraphicsDevice.Viewport.TitleSafeArea.Right - 100;
    ...
    private void StartAnimations()
    {
      ...
      AnimationService.StartAnimation(_oscillatingXAnimation, _animatableX).AutoRecycle();
      AnimationService.StartAnimation(_loopedColorAnimation, _animatableColor).AutoRecycle();
    }
